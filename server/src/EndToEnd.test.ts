...
test("post /event/authorized/organisation/:orgId", async () =>{
	...
	//let testUser2 that does not have perm for orgId = 2 try add an event
	...
	//let testUser1 add an event with perm for orgId = 2 but not sending all data
	const res4 = await request.post("/event/authorized/organisation/2").set('Authorization', token2).send(event);
	expect(res4.statusCode).toBe(400);

	//let testUser1 add an event with perm for orgId = 2 and full data
	event.title = "Robot"
	...
	//let testUser1 add an event with perm for orgId = 4 and full data  
